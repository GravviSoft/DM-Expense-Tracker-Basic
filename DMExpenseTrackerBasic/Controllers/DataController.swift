...
enum DataController {
    static var expenseData: [Expense] = []
    
    static var expenseTotalLabel: String = ""
    ...
    static func configureTableView() -> Set<String> {
        let categories = data.map { $0.category }
        return Set(categories)
    }
    
    static func initializeExpenseData(cat: String) -> [Expense]{
        expenseData = data.filter({ $0.category == cat})
        let total = expenseData.map { $0.amount }
        expenseTotalLabel = "You spent $\(total.reduce(0, +)) on \(cat)."
        return expenseData
    }
}
